...
def temporal_cv_folds(df, year_col='Year', n_folds=5):
    """
    Genera índices de train/val para validación cruzada temporal en panel de años.
    Devuelve una lista de tuplas (train_idx, val_idx) para usar en cross-validation.
    """
    years = sorted(df[year_col].unique())
    fold_size = len(years) // n_folds
    folds = []
    for i in range(n_folds):
        val_years = years[i*fold_size:(i+1)*fold_size] if i < n_folds-1 else years[i*fold_size:]
