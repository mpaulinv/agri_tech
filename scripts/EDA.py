...
### Hay un conflicto por el nombre de la variable hg/ha_yield. Renombramos 

# Renombrar 'hg/ha_yield' por 'yield' para el análisis
maize_data = maize_data.rename(columns={'hg/ha_yield': 'yield'})

variables = ['yield', 'average_rain_fall_mm_per_year', 'pesticides_tonnes', 'avg_temp']


def summary_year_area(data, variable, output_path):
    """
    Genera una tabla con el mínimo, máximo y promedio de una variable por año y la guarda en un CSV.

    Parameters:
    - data: DataFrame con los datos
    - variable: str, nombre de la variable continua a analizar
    - output_path: str, ruta donde guardar el CSV
    """
    resumen = data.groupby('Area')[variable].agg(['min', 'max', 'mean']).reset_index()
    resumen.to_csv(output_path, index=False)
    print(f"Tabla de resumen guardada en {output_path}")
    print(resumen)



for var in variables:
    summary_year_area(maize_data, var, f'outputs/{var}_summary_year_area.csv')
...
### en particular, hay valores muy altos (alredor de 12 veces la mediana) para el yield. Los países que muestran niveles más altos de Yield con diferencias 
### altas entre máximos y mínimos son Qatar, Tajikistan y Netherlands. Errores o outliets en la variable target pueden afectar las métricas del modelo o sesgar los parámetros. 
### 5) No hay variacion por año para la variable de lluvia. Esto es un defecto de la data y posiblemente afecte al modelo. Aquí me gustaría revisar con 
### el equipo que genera o carga la data para ver si esto se puede corregir ya que ayudaría mucho a mejorar las predicciones del modelo.
